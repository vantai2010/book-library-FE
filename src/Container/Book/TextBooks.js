...
export default function TextBooks() {
    ...
    const [listTextBooks, setListTextBooks] = useState([])
    const language = useSelector(state => state.app.language)

    const handlegetTextBooks = async () => {
        let res = await getAllBookByCategorySerive({ categoryId: category_book.TEXTBOOKS, limit: 8 })
        if (res && res.data && res.data.errCode === 0) {
            setListTextBooks(res.data.data)
        } else {
    ...
    const handleRedirectToListBook = () => {
        localStorage.setItem(SCROLL_BACK_LOCATION, window.scrollY)
        localStorage.setItem(NAME_BACK_LOCATION, '/home')
        navigate(`/list-books/${category_book.TEXTBOOKS}/20`)
    }

    return (
        <>
            {
                listTextBooks && listTextBooks.length > 0 &&
