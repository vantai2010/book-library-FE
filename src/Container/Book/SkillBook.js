...
export default function SkillBook() {
    ...
    const [listSkillBook, setListSkillBook] = useState([])
    const language = useSelector(state => state.app.language)

    const handlegetSkillBook = async () => {
        let res = await getAllBookByCategorySerive({ categoryId: category_book.SKILLBOOK, limit: 8 })
        if (res && res.data && res.data.errCode === 0) {
            setListSkillBook(res.data.data)
        } else {
    ...
    const handleRedirectToListBook = () => {
        localStorage.setItem(SCROLL_BACK_LOCATION, window.scrollY)
        localStorage.setItem(NAME_BACK_LOCATION, '/home')
        navigate(`/list-books/${category_book.SKILLBOOK}/20`)
    }

    return (
        <>
            {
                listSkillBook && listSkillBook.length > 0 &&
