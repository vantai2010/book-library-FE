...
export default function ComicBook() {
    ...
    const [listComicBook, setListComicBook] = useState([])
    const language = useSelector(state => state.app.language)

    const handlegetComicBook = async () => {
        let res = await getAllBookByCategorySerive({ categoryId: category_book.COMIC, limit: 8 })
        if (res && res.data && res.data.errCode === 0) {
            setListComicBook(res.data.data)
        } else {
    ...
    const handleRedirectToListBook = () => {
        localStorage.setItem(SCROLL_BACK_LOCATION, window.scrollY)
        localStorage.setItem(NAME_BACK_LOCATION, '/home')
        navigate(`/list-books/${category_book.COMIC}/20`)
    }
    return (
        <>
            {
                listComicBook && listComicBook.length > 0 &&
